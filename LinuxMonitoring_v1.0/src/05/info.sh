...
function folders {
    ...
    echo "Total number of folders (including all nested ones) = $file_caunt"
    echo "TOP 5 folders of maximum size arranged in descending order (path and size):"
    du -h $1 2>/dev/null | sort -hr | head -5 | awk 'BEGIN{i=1}{printf "%d - %s, %s\n", i, $2, $1; i++}'
    echo "etc up to 5"
}

function files_caunt {
    ...
    arh="$(find $1 -type f -name "*.zip" -o -name "*.7z" -o -name "*.rar" -o -name "*.tar" | wc -l)"
...
function execution_time {
  ...
  echo "Script execution time (in seconds) = $(($end-$begin_time))"
}
